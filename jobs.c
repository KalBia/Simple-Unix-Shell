...
/* DESCRIPTION:
 * Find an empty slot in jobs' array - make it bigger if needed
 * INPUT:
 * none
 * OUTPUT:
 * static int - index of allocated job in array */
...
/* DESCRIPTION:
 * Find place for process in the processes' array in job nr j
 * INPUT:
 * int j - number of job that process works for
 * OUTPUT:
 * static int - index of allocated process in array */
...
/* DESCRIPTION:
 * Initialize new job in jobs' array
 * INPUT:
 * pid_t pgid - job's process group ID
 * int bg - foreground (0) or background (1)
 * OUTPUT:
 * int - index of job in jobs' array */
...
/* DESCRIPTION:
 * Empty the slot in jobs' array to reuse it
 * INPUT:
 * job_t *job - pointer to the job to delete
 * OUTPUT:
 * none */
...
/* DESCRIPTION:
 * Move job from one slot to another
 * INPUT:
 * int from - number of slot from which we move job
 * int to - number of slot to which we move job
 * OUTPUT:
 * none */
...
/* DESCRIPTION:
 * Make string with command from shell - uses strapp from lexer.c
 * INPUT:
 * char **cmdp - pointer to poiner to where we want to have the string
 * char **argv - arguments as we put them in execve
 * OUTPUT:
 * none */
...
/* DESCRIPTION:
 * Add process to the processes' array in job
 * INPUT:
 * int j - number of job, "owner" of process
 * pid_t pid - pid of process to add
 * char **argv - args like in execve
 * OUTPUT:
 * none */
 ...
 * INPUT:
 * int j - number of job
 * int *statusp - pointer to place for exitcode, if state == finished
 * OUTPUT:
 * static int - job's state */
 ...
 * INPUT:
 * int j - number of job, can be -1 => we take the job with highest number
 * int bg - foreground (1) or background (1)
 * sigset_t *mask
 * OUTPUT:
 * bool - success? */
 ...
 * INPUT:
 * int which - ALL / FINISHED / RUNNING / STOPPED
 * OUTPUT:
 * none */
